...
- **Staged-update progress rows are capped.** `onBootcStageClicked` and `onSysupdateStageClicked` (`internal/views/updates_page.go`) append one `ActionRow` per `EventMessage` to the Details expander; without a cap, verbose staging accumulates thousands of heavyweight widgets and freezes the main thread. Retention is bounded by `internal/progresslog.Buffer` (`internal/progresslog`, `DefaultCap` = 200): each message runs through `Buffer.Add`, which reports an overflow once the cap is exceeded, and the handler drops the oldest row from the expander so the row count tracks the messages the buffer keeps. The buffer is widget-free and headlessly tested, because `internal/views` cannot host a test binary.
